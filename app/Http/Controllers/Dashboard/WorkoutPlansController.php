    ...
    public function store($member_id, Request $request)
    {
        ...
        if ($member) {
            ...
            if ($user) {
                ...
                if (WorkoutPlans::where('member_id', $member->id)->where('user_id', $user->id)->count() > 0) {
                    $plan = WorkoutPlans::where('member_id', $member->id)->where('user_id', $user->id)->first();
